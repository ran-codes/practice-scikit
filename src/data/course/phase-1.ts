...
export const phase1Exercises: CourseExercise[] = [
  {
    ...
    description:
      "Learn how to load built-in datasets from sklearn.datasets. These datasets are perfect for learning and prototyping.",
    difficulty: "easy",
    concepts: ["datasets", "load_iris", "load_digits", "data structure"],
    hints: [
      "Use load_iris() to get the iris dataset",
      "The returned object has .data, .target, and .feature_names attributes",
    ],
...
print("Iris dataset keys:", dir(iris))
print()

# TODO: Print the shape of the data and target
print("Data shape:", None)  # <- iris.data.shape
print("Target shape:", None)  # <- iris.target.shape
print()

# TODO: Print the feature names
print("Feature names:", None)  # <- iris.feature_names
print()

# TODO: Print the target names (species)
print("Target names (classes):", None)  # <- iris.target_names

# Set result for verification
result = f"Loaded iris with {iris.data.shape[0]} samples and {iris.data.shape[1]} features"
result`,
  },
  {
    id: "exploring-data-pandas",
    type: "course",
    phase: 1,
    order: 2,
    title: "Exploring Data with Pandas",
    description:
      "Convert sklearn datasets to pandas DataFrames for easier exploration and analysis.",
    difficulty: "easy",
    concepts: ["pandas", "DataFrame", "describe", "value_counts"],
    hints: [
      "Use pd.DataFrame(data, columns=feature_names) to create a DataFrame",
...
# TODO: Create a DataFrame from the iris data with proper column names
df = pd.DataFrame(None, columns=None)  # <- use iris.data and iris.feature_names

# TODO: Add the target column
df['species'] = None  # <- use iris.target
...
print(None)  # <- use df.describe()
print()

# TODO: Count samples per species
print("Samples per species:")
print(None)  # <- use df['species'].value_counts()
...
education_encoded = None  # <- le_education.fit_transform(data['education'])
...
city_encoded = None  # <- le_city.fit_transform(data['city'])

# Create encoded dataframe
data_encoded = data.copy()
data_encoded['education'] = education_encoded
data_encoded['city'] = city_encoded
